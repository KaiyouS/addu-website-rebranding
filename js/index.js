function formatDate(isoDateString, format) {
    ...
    if (format === 'month') {
        return date.toLocaleDateString('en-US', { month: 'short', timeZone: 'UTC' }); // Returns 3-char month
    } else if (format === 'day') {
        return date.toLocaleDateString('en-US', { day: 'numeric', timeZone: 'UTC' }); // Returns day portion
    }
    return date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' }); // Default format
}
                ...
                academicYear.terms.forEach(term => {
                    term.events.forEach(event => {
                        if (event.date) {
                            ...
                            const eventStartDate = new Date(event.dateRange.start + 'T00:00:00Z');
                ...
                if (event.date) {
                    accordionDate = `
                        <time datetime="${event.date}">
                            <div class="month">${formatDate(event.date, 'month')}</div>
                            <div class="day">${formatDate(event.date, 'day')}</div>
                        </time>
                    `;
                } else if (event.dateRange) {
                    accordionDate = `
                        <time datetime="${event.dateRange.start}">
                            <div class="month">${formatDate(event.dateRange.start, 'month')}</div>
                            <div class="day">${formatDate(event.dateRange.start, 'day')}-${formatDate(event.dateRange.end, 'day')}</div>
                        </time>
                    `;
                }
