  ...
  accordionItems.forEach((item) => {
    ...
    if (trigger && content) {
      const contentInner = content.querySelector(
        ".calendar-accordion-content-inner"
      );
    ...
    if (data.academicYears && data.academicYears.length > 0) {
      data.academicYears.forEach((academicYear) => {
        ...
        academicYear.terms.forEach((term) => {
          ...
          container.className = "calendar-container open";

          const calendarTitleHTML = `
            <div class="calendar-header">
              <h2 class="calendar-title">${term.name}</h2>
          ...
          itemsWrapper.className = "calendar-accordion-items-wrapper";
          ...
          term.events.forEach((event) => {
            ...
            if (event.description) {
              descriptionHTML = `
                <div class="calendar-accordion-content" aria-hidden="true" style="height: 0px;">
              ...
              <div class="calendar-accordion-item">
                <div class="calendar-accordion-header">
                  <div class="calendar-accordion-labels">
                    <span class="calendar-accordion-date">${accordionDate}</span>
                    <span class="calendar-accordion-label"><span>${event.label}</span></span>
                  </div>
                  ${buttonHTML}
                </div>
                ${descriptionHTML}
            ...
            itemsWrapper.insertAdjacentHTML("beforeend", eventItemHTML);
          });
          ...
          header.addEventListener("click", () => {
            ...
            const itemsWrapper = container.querySelector(".calendar-accordion-items-wrapper");
